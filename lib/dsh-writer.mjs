 ...
 *
 * NEVER rewrite a session by compressing the whole JSONL into one zstd
 * frame (`zstd -19 file`). DSH scans require the first frame to contain
 * exactly one header line (`first frame is not exactly one header line`).
 * Repair in place: keep a backup, then header-frame + events-frame, and
 * verify the decompressed SHA-256 is unchanged.
 */
 ...
 * @param header - { id, createdAt, cwd, agentPreset? } (id must be a stable import id).
...
export function writeSession(root, header, events) {
  const headerLine = {
    ...
    delegationDepth: 0,
    agentPreset: header.agentPreset || 'cordis',
  }
