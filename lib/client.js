...
const styles = {
  ...
  /**
   * Floating result popover: absolutely positioned above the button, so it
   * NEVER takes layout space in the composer row (a long import summary used
   * to push the whole row left and overlap the permissions control).
   */
  popover: {
    position: 'absolute',
    bottom: 'calc(100% + 6px)',
...
function SyncMenu({ runCommand }) {
  ...
  return h('span', { ref: rootRef, style: styles.wrapper },
    ...
    open
      ...
      : null,
    // floating popover above the button — takes no layout space, so the
    // composer row never shifts (hidden while the menu is open: the badge
    // and menu itself are the feedback for the toggle)
    !open && (busyLabel !== null || result !== undefined)
      ? h('div', {
          style: Object.assign(
            {},
            styles.popover,
            busyLabel !== null ? styles.ok : result.ok ? styles.ok : styles.error,
          ),
        }, busyLabel !== null ? busyLabel : result.text)
      : null,
  )
}
