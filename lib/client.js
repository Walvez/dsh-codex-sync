...
function ImportPicker({ lang, runCommand, onClose }) {
  ...
  const renderNodes = (nodes, depth) => nodes.map((n) => {
    ...
    return h('div', { key: n.id },
      h('div', {
        style: Object.assign({}, styles.treeRow, locked ? styles.muted : {}, { paddingLeft: 6 + depth * 16 }),
      },
        h('button', {
          type: 'button',
        ...
        h('button', {
          type: 'button',
          disabled: locked,
          style: { border: 'none', background: 'transparent', padding: 0, cursor: locked ? 'default' : 'pointer' },
          onClick: () => {
            if (locked) return
        ...
        n.stale ? h('span', { style: styles.importedTag }, t.updated)
