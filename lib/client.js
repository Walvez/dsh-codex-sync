...
/**
 * Second-level management page (inside the settings dialog): the feature
 * master switch (enableSkills / mcpMirror) on top, then a search box, then
 * ItemSyncPanel. Used by both the skill and MCP managers.
 */
function ItemsManagerPage({ lang, kind, itemsData, itemsLoading, settings, toggleSetting, onToggleItem, onSetAll }) {
  const t = labels(lang)
  const [query, setQuery] = useState('')
  ...
  const masterKey = kind === 'skill' ? 'enableSkills' : 'mcpMirror'
  ...
  return h('div', {
    ...
    h('div', {
      ...
      h('div', { key: 'main', style: Object.assign({}, styles.dialogBody, { padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: '16px' }) },
        // Section 1: Actions. Row 1: managers (技能管理 / MCP 管理).
        // Row 2: import/export. Row 3: status/refresh.
        h('div', { style: styles.modalSection },
          h('div', { style: styles.modalSectionTitle }, t.actions),
          h('div', { style: styles.actionGrid },
            // Skill manager — second-level page, same interaction as import/export.
            h('button', {
              type: 'button',
              className: 'codex-sync-card',
              style: Object.assign({}, styles.actionCard, { width: '100%' }),
              onClick: () => setSubpage('skill'),
            },
              h('span', { style: styles.actionCardIcon }, Icons.codex(16)),
              h('span', { style: styles.actionCardLabel }, t.skillsPageTitle),
              (() => {
                const n = itemsData ? Object.keys(itemsData.skills?.items ?? {}).length : null
                return typeof n === 'number' && n > 0
                  ? h('span', { style: Object.assign({}, styles.itemBadge, { marginLeft: 'auto' }) }, t.expandHint(n))
                  : h('span', { style: styles.spacer })
              })()
            ),
            // MCP manager — same pattern.
            h('button', {
              type: 'button',
              className: 'codex-sync-card',
              style: Object.assign({}, styles.actionCard, { width: '100%' }),
              ...
              h('span', { style: styles.actionCardLabel }, t.mcpsPageTitle),
              (() => {
                const n = itemsData ? Object.keys(itemsData.mcps?.items ?? {}).length : null
                return typeof n === 'number' && n > 0
                  ? h('span', { style: Object.assign({}, styles.itemBadge, { marginLeft: 'auto' }) }, t.expandHint(n))
                  : h('span', { style: styles.spacer })
              })()
...
function SyncMenu({ runCommand, wide }) {
  ...
  const handleOpenPath = async (target) => {
    ...
    if (!ok && typeof runCommand === 'function') {
      void run('refresh', '/codex-settings')
    }
  }
