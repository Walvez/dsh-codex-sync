...
import { effectiveItemSync } from './state.js'

/** @type {Promise<object | undefined> | undefined} */
let mcpClientModulePromise

/**
 * Load the mcp-client package once (peer optional).
 *
 * Deliberately NOT a bare-specifier dynamic import: Node negatively caches a
 * failed specifier resolution for the whole process, so a package installed
 * later (runtime repair, test fixtures) would never be found again. Resolving
 * through createRequire().resolve() on every attempt and importing the file
 * URL keeps retry semantics; the successful MODULE is still cached.
 */
const requireResolve = createRequire(import.meta.url).resolve
function loadMcpClient() {
  if (mcpClientModulePromise === undefined) {
    mcpClientModulePromise = Promise.resolve()
      .then(() => {
        let resolved
        if (process.env.DSH_CX_SYNC_MCP_CLIENT_DIR !== undefined) {
          // Test seam: resolve from an explicit fixture directory (node --test
          // runs test FILES concurrently; each file owns its own fixture).
          resolved = createRequire(join(process.env.DSH_CX_SYNC_MCP_CLIENT_DIR, 'package.json'))
            .resolve('@deepseek-ai/dsh-mcp-client')
        } else {
          resolved = requireResolve('@deepseek-ai/dsh-mcp-client')
        }
    ...
    // A failed load resolves to `undefined`; reset so later syncs can retry.
    mcpClientModulePromise.then((mod) => {
      if (mod === undefined) mcpClientModulePromise = undefined
    }, () => { mcpClientModulePromise = undefined })
  }
...
/** Read-only view of the hard-deny set for UI/command layers. */
export function hardDenyNames() {
  return new Set(HARD_DENY)
}
...
/**
 * Render a mirror status snapshot (from `startMcpMirror().getStatus()`) as
 * the /mcp-status command text. Pure function — shared by the handler and
 * the test suite.
 * @param {{configPath: string, enabled: boolean, lastSyncAt: string|null, servers: Array<object>}} status
 * @returns {string}
 */
export function formatMcpStatus(status) {
  const lines = [
    `MCP mirror: ${status.enabled ? 'on' : 'off'}  ·  ${status.configPath}`,
    ...(status.lastSyncAt ? [`last sync: ${status.lastSyncAt}`] : ['last sync: (initial mount in progress…)']),
    '',
  ]
  const byName = [...status.servers].sort((a, b) => a.name.localeCompare(b.name))
  for (const s of byName) {
    const flags = [
      s.silent ? 'silent' : null,
      s.reason === 'mounted' ? '✓' : null,
    ].filter(Boolean).join(' ')
    const detail = s.reason === 'failed' ? ` (${s.error ?? 'unknown error'})` : ''
    lines.push(`${s.name.padEnd(16)} ${s.transport.padEnd(15)} ${s.reason}${flags ? ' ' + flags : ''}${detail}`)
  }
  lines.push('', `servers: ${byName.length}`, 'hint: /codex-mcp <名> on|off · codex config edits live-sync')
  return lines.join('\n')
}
...
export function startMcpMirror(ctx, codexHome = join(homedir(), '.codex'), config = {}) {
  ...
  async function sync() {
    ...
    for (const [name, srv] of parsed) {
      ...
      // Per-server sync preference (UI / /codex-mcp): user-off hides the
      // mirror without touching codex config. Read live so toggles hot-apply.
      if (!effectiveItemSync('mcp', name)) {
        status.set(name, { ...base, reason: 'user-disabled' })
        continue
      }
      ...
      wanted.set(name, cfg)
      // Only brand not-yet-mounted servers as 'mounting'; an already-mounted
      // server that survives this pass must keep its 'mounted' status even
      // though the mount loop below skips it.
      if (!mounted.has(name)) status.set(name, { ...base, reason: 'mounting' })
    }
  ...
  /** Serialize sync() calls: the initial mount races refresh() calls otherwise. */
  ...
  try {
    watcher = watch(configPath, () => {
      ...
      debounce = setTimeout(() => void enqueueSync(), 500)
    })
  } catch {
  ...
  return {
    ...
    getStatus,
    /** Re-run sync immediately (serialized) — per-item toggles hot-apply. */
    refresh: () => enqueueSync(),
  }
}
