...
export function registerCodexSkillProvider(codexHome, config, control, enabled = () => true) {
  ...
  function scan() {
    ...
    for (const entry of entries) {
      ...
      // Per-skill sync preference: an explicit off hides the skill entirely.
      // Checked on the raw entry name; state keys use the same raw string the
      // UI lists, so renamed folders start fresh under the group default.
      if (!effectiveItemSync('skill', entry)) continue
